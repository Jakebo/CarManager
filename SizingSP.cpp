...
void CSizingSP::CreateSizeGrip()
{
	...
	m_hSizeGrip = ::CreateWindow("ScrollBar", "",
...
BOOL CSizingSP::GetRatesFromSec(LPCTSTR lpszSec, HWND hCtrl, double* pdRate) const
{
		...
		|| strWord == _T("RX") || strWord == _T("RY"))
	{
		...
		if (strWord != _T("RY"))
		{
			pdRate[0] = dCoef*(rcInitCtrl.left  - m_rcInitClt.left)
				/(m_rcInitClt.right - m_rcInitClt.left);
			pdRate[1] = dCoef*(rcInitCtrl.right - m_rcInitClt.left)
				/(m_rcInitClt.right - m_rcInitClt.left);
		}
		if (strWord != _T("RX"))
		{
			pdRate[2] = dCoef*(rcInitCtrl.top    - m_rcInitClt.top )
				/(m_rcInitClt.bottom - m_rcInitClt.top);
			pdRate[3] = dCoef*(rcInitCtrl.bottom - m_rcInitClt.top )
				/(m_rcInitClt.bottom - m_rcInitClt.top);
		}
	}
	...
	{
		CString strErr;
		strErr.Format(_T("'%s' is not a predefined keyword."), strWord);
...
// or "(-12.36R)"
void CSizingSP::GetCoefficientAndWord(
...
void CSizingSP::GetWindowTextSize(HWND hWnd, SIZE& sz) const
{
	...
	if (str[0] == '\0')
		::strcpy(str, _T("a"));		// To avoid getting zero valued sz.
